...
public class PassengerCounter : MonoBehaviour
{
    private void Start()
    {
        GameResources.OnStationAmountChanged += UpdateResourceTextObject;
        UpdateResourceTextObject(null, EventArgs.Empty); // Call the method manually to initialize the text
    }
    ...
    private void UpdateResourceTextObject(object sender, EventArgs e)
    {
        var TotalResources = GameResources.GetStationAmount(GameResources.StationType.Red) +
                             GameResources.GetStationAmount(GameResources.StationType.Blue) +
                             GameResources.GetStationAmount(GameResources.StationType.Yellow);
        transform.Find("passengerAmount").GetComponent<Text>().text = TotalResources.ToString();
    }
